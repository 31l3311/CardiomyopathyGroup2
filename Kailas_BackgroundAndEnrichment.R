...
# threshold kept at 1 fpkm. cutOffPercentage = 40.
# This means we discard genes which have at least 40% of the replicates in each disease type
# showing an expression lower than 1 fpkm. 15682 genes left after this.
...
write.table(ABgxData, file = "D:/Documents/Github/Research_project_1/ABgxData.txt", row.names = TRUE, quote = FALSE, sep ="\t")
write.table(ABgeneTotExonLengths, file = "D:/Documents/Github/Research_project_1/ABgeneTotExonLengths.txt", row.names = TRUE, quote = FALSE, sep ="\t")
#-------------------------------------------------------------------------------
# Old background correction method
#-------------------------------------------------------------------------------

# Finding all Y-chromosome genes
# yGeneID <- getBM(attributes = c("ensembl_gene_id"),
...
# backgroundTest <- c()
# for (i in 1:nrow(gxData_fpkm)) {
#  backgroundTest[i] <- rmeans[i] >= background
# }
# a=gxData[backgroundTest,]
...
EntrezIDs <- distinct(EntrezIDs, ENSEMBL, .keep_all = TRUE) # keep unique ensembl IDs
...
#                 mart = ensembl
# )
# data.frame(table(geneInfo$gene_biotype))
...
# design <- model.matrix(~ 0 + Disease + Sex + Ethnicity ,data = pData(eset)) #design matrix which does not look at differences in sex.
design <- model.matrix(~ 0 + disease.sex + Ethnicity, data = pData(eset))

# Defining contrasts
# cont2 <- makeContrasts(  # OLD CONTRASTS WITHOUT INTERACTION CONTRASTS
#  DCMvDon = DiseaseDCM - DiseaseDonor,
#  HCMvDon = DiseaseHCM - DiseaseDonor,
#  PPCMvDon = DiseasePPCM - DiseaseDonor, levels = design)
...
# Differentially expressed genes in the three contrasts - logFC threshold was kept to log2(1.5).
...
# Should we add interaction DEGs to this?
...
write.table(ABgxDataDEG, file = "DEG data.txt", sep = "\t", quote = FALSE, row.names = TRUE)
...
# dataframe of significant DEGs HCM (entrez gene name, logfc, adjusted pvalue)
...
# dataframe of significant DEGs PPCM (entrez gene name, logfc, adjusted pvalue)
...
#-------------------------------------------------------------------------------
# Creating dataframe for each disease of all DEGs- gene symbol,logfc, adj.p.value
#-------------------------------------------------------------------------------
adj.p <- list()
foldChange <- list()
geneName <- list()
...
allDEgenes_DCM <- foldChange[, c(1, 2, 3)]
allDEgenes_HCM <- foldChange[, c(4, 5, 6)]
allDEgenes_PPCM <- foldChange[, c(7, 8, 9)]
allDEgenes_INTdcm <- foldChange[, c(10, 11, 12)]
allDEgenes_INThcm <- foldChange[, c(13, 14, 15)]

#-------------------------------------------------------------------------------
# Annotating the allDEgenes_ with gene symbols
# Creating inputLists for pathfindR
# Creating expression matrix with gene symbols. Some genes
# do not have gene symbols available, resulting matrix dimension smaller.
#-------------------------------------------------------------------------------
# input =  must be a data frame with columns containing: Gene Symbols, Change Values (optional) and p values.
diseases <- c("DCM", "HCM", "PPCM", "INTdcm", "INThcm")
for (i in diseases) {
  source <- get(paste("allDEgenes_", as.character(i), sep = ""))
...
inputList_DCM[, c(3, 4)] <- (allDEgenes_DCM[allDEgenes_DCM$EnsemblID %in% inputList_DCM[, 1], c(2, 3)])
...
inputList_HCM[, c(3, 4)] <- (allDEgenes_HCM[allDEgenes_HCM$EnsemblID %in% inputList_HCM[, 1], c(2, 3)])
...
inputList_PPCM[, c(3, 4)] <- (allDEgenes_PPCM[allDEgenes_PPCM$EnsemblID %in% inputList_PPCM[, 1], c(2, 3)])
...
inputList_INTdcm[, c(3, 4)] <- (allDEgenes_INTdcm[allDEgenes_INTdcm$EnsemblID %in% inputList_INTdcm[, 1], c(2, 3)])
...
inputList_INThcm[, c(3, 4)] <- (allDEgenes_INThcm[allDEgenes_INThcm$EnsemblID %in% inputList_INThcm[, 1], c(2, 3)])
...
#
# pathfindr - needs gene symbol, fc, and adj p vals 
#-------------------------------------------------------------------------------

# run enrichment analysis on each disease set using KEGG gene set and KEGG protein-protein interaction network
# gene_sets available - "KEGG", "Reactome", "BioCarta", "GO-All", "GO-BP", "GO-CC", "GO-MF", "cell_markers"
# protein-protein interaction network available - "Biogrid", "STRING", "GeneMania", "IntAct", "KEGG"

dir <- ""
for (i in c("inputList_PPCM")){#, "inputList_HCM")) { # , "inputList_PPCM","inputList_INTdcm","inputList_INThcm")){
  if (i == "inputList_DCM") {
...
#-------------------------------------------------------------------------------
# results of pathfindr and visualising
#-------------------------------------------------------------------------------

UpSet_plot(pathfindResult_DCM,num_terms = 20)
...
for (i in c("PPCM")){#, "DCM", "HCM")) {
  enrichment <- get(paste("pathfindResult_",i,sep=""))
...
score_matrix_DCM <- score_terms(
  enrichment_table = pathfindResult_DCM[pathfindResult_DCM$Status == "Representative", ],
  exp_mat = as.matrix(GeneSymbols),
  cases = casesDCM,
  #control_title = "healthy", # default = "Control"
  #use_description = TRUE, # default FALSE
...
#-------------------------------------------------------------------------------
# creating data for webgestalt
#-------------------------------------------------------------------------------

# df of ordered FC with names (ensembl) - for webgestalt GSEA
dcmVectorGSEA <- DEGdcmEnsembl[, c(1, 2)]
hcmVectorGSEA <- DEGhcmEnsembl[, c(1, 2)]
ppcmVectorGSEA <- DEGppcmEnsembl[, c(1, 2)]
...
# topGO
#-------------------------------------------------------------------------------
# Vector of interesting ensembl IDs (such as DE genes). if its ensembl, make sure to convert to entrez later.
InterestingGenesforTopGO <- ppcmDEensembl

# converting ensembl to entrez
for (ont in c("BP", "CC", "MF")) {
  print(ont)
  dataForTopGO <- annFUN.org(ont, mapping = "org.Hs.eg.db", ID = "ensembl") # in place of "BP"- "MF" and "CC" are also valid
...
hist(score(goFisherResult), 5, xlab = "p-values")

# The subgraph induced by the top5GO terms identified by the classic algorithm for
# scoring GO terms for enrichment.  Rectangles indicate the5most significant terms.
...
goID <- allRes[1, "GO.ID"]
print(showGroupDensity(GOdata, whichGO = goID, ranks = TRUE))

printGraph(GOdata, goFisherResult, firstSigNodes = 5, fn.prefix = "tGO", useInfo = "all", pdfSW = F)
